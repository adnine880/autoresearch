...
# Usage: bash run_experiment.sh "<description>"
# Example: bash run_experiment.sh "depth 10"
#
# Outputs one of:
#   CRASH
...
if [ -z "$VAL_BPB" ]; then
    echo "CRASH"
    bash log_result.sh 0.000000 0.0 crash "$DESC"
    exit 1
fi
...
if [ "$IMPROVED" = "1" ]; then
    ...
    bash log_result.sh "$VAL_BPB" "$MEMORY_GB" keep "$DESC"
    echo "IMPROVED $VAL_BPB $MEMORY_GB"
else
    bash log_result.sh "$VAL_BPB" "$MEMORY_GB" discard "$DESC"
